...
export const BoardRow = ({ title, subtitle, cards, selectedId, attackMode = false, onCardClick, onCardHover }: BoardRowProps) => {
  ...
  return (
    <section className="rounded-[1.6rem] border border-brass-700/30 bg-black/18 p-3 sm:p-3.5 shadow-insetPanel backdrop-blur-xl">
      ...
      <div className="grid min-h-[8.4rem] grid-cols-2 gap-2 sm:min-h-[10rem] sm:gap-3 md:grid-cols-3 xl:grid-cols-5">
        <AnimatePresence>
        ...
        {Array.from({ length: emptySlots }, (_, index) => (
          <div key={`slot-${title}-${index}`} className="rounded-[1.1rem] sm:rounded-[1.35rem] border border-dashed border-brass-100/10 bg-white/[0.015] min-h-[8rem] sm:min-h-[10rem]" />
        ))}
      </div>
    </section>
  );
